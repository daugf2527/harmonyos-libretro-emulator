...
sdk_root="${tool_home}/sdk"
sdk_default_root="${sdk_root}/default"
openharmony_root="${sdk_default_root}/openharmony"
hms_root="${sdk_default_root}/hms"
...
print_sdk_diagnostics() {
  local sdk_root="${sdk_default_root}"
  local oh_root="${openharmony_root}"
  local hms_sdk_root="${hms_root}"

  echo "[INFO] SDK diagnostics: sdk root = ${sdk_root}"
  echo "[INFO] SDK diagnostics: DEVECO_SDK_HOME = ${deveco_sdk_home}"
  echo "[INFO] SDK diagnostics: HARMONY_COMMANDLINE_TOOLS_HOME = ${tool_home}"
  echo "[INFO] SDK diagnostics: HARMONY_SDK_TOOLCHAINS = ${sdk_toolchains}"

  echo "[INFO] SDK root directories"
  find "${deveco_sdk_home}" -maxdepth 1 -mindepth 1 -type d | sort || true
  ...
  if [[ -d "${hms_sdk_root}" ]]; then
    echo "[INFO] HMS component directories"
    find "${hms_sdk_root}" -maxdepth 1 -mindepth 1 -type d | sort || true

    for component in native ets js toolchains previewer; do
      local component_dir="${hms_sdk_root}/${component}"
      local manifest="${component_dir}/uni-package.json"
      if [[ -d "${component_dir}" ]]; then
        echo "[INFO] HMS component ${component}: present"
        du -sh "${component_dir}" || true
        if [[ -f "${manifest}" ]]; then
          echo "[INFO] hms/${component} manifest"
          cat "${manifest}"
        else
          echo "[WARN] hms/${component} manifest missing: ${manifest}"
        fi
        echo "[INFO] hms/${component} sample files"
        find "${component_dir}" -maxdepth 2 -type f | sort | head -n 20 || true
      else
        echo "[WARN] HMS component ${component}: missing directory ${component_dir}"
      fi
    done
  else
    echo "[WARN] HMS root missing: ${hms_sdk_root}"
  fi
}
