...
> 日期：2026-04-30
> 范围：`docs/design/stitch-game-emulator-plan/_1` 到 `_15`。
...
### 视觉 gap
- 已按 `_5` 关于帮助主态收口：hero 状态行、大标题、说明文案、版本卡进度条、帮助列表、右箭头、footer 分割线、底部品牌块和 System 导航高亮已对齐首批视觉目标。
- 顶部与版本信息保持本地 about/build 语义，`Status` 改为 `LOCAL_ABOUT_READY`，不写成已验证的运行态在线服务状态。

### 交互 gap
- 帮助项点击只切换本地展开态，不跳在线文档、反馈或法律外链；未接入的远端资料继续保持 `LOCAL_ONLY / NOT_CONFIGURED` 边界。
- 底部导航和 System 高亮已与 Settings/Core 分组统一；实际路由点击仍需设备验证。
