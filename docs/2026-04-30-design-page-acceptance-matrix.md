...
| `stitch_game_emulator_design_plan/_10` | `entry/src/main/ets/pages/LibraryDetailPage.ets`、`entry/src/main/ets/components/LibraryDetailHeroPanel.ets`、`entry/src/main/ets/components/LibraryDetailInfoPanel.ets`、`entry/src/main/ets/components/LibraryLaunchOverlay.ets` | 部分完成 | 已有详情页、hero、metadata、launch overlay 和 runtime snapshot；仍需对齐封面比例、绿色 CTA、metadata grid、telemetry 和 overview。 | Library -> Detail -> Runtime 参数、launch overlay、编辑元数据和 quick actions 需按真实链路复核。 | `LOCAL_ONLY` / `NOT_CONFIGURED` / `需真机验证`：云存档未配置，启动和 runtime 写回需设备验证。 | `# _10 Library Detail` | 第四组收口：先保真实 runtime 数据，再做 loading/launch 视觉。 |
