...
| `stitch_game_emulator_design_plan/_13` | `entry/src/main/ets/pages/ImportEntryPage.ets`、`entry/src/main/ets/components/ImportEmptyStateHero.ets` | 视觉完成 | 2026-05-04 对照：`03_import_authorization_required.png` 是授权 step 2，和 `_13/screen.png` 的导入空态 step 1 不同；`01/04` 只提供库页入口辅助证据。 | 导入按钮仍只做真实 picker/import service handoff；pre-picker 不再强化假网络在线状态。 | `LOCAL_ONLY` / `需真机验证`：已有授权态截图；step 1 导入空态、picker、任务页跳转和库刷新仍需设备验证。 | `docs/2026-05-04-artifact-to-runtime-gap-audit.md#_13-import-entry` | 补 `_13` step 1 导入空态和 picker/任务创建截图；不升级运行态完成。 |
| `stitch_game_emulator_design_plan/_14` | `entry/src/main/ets/pages/ImportTaskOverlayPage.ets`、`entry/src/main/ets/common/RomImportService.ets`、`entry/src/main/ets/common/ImportTaskBridge.ets` | 视觉完成 | 2026-05-04 对照：`05_import_task_empty_dialog.png` 是 `NO_PENDING_IMPORT_TASK` 空态，不是 `_14/screen.png` 的 importing 进度态；弹层宽度、dim/blur 层级可见。 | cancel/retry/return/conflict 处理继续来自真实 import task；无 pending task 时保持 `NO_PENDING_IMPORT_TASK` 诚实空态。 | `LOCAL_ONLY` / `需真机验证`：已有无任务空态截图；有 pending task、复制、冲突、取消、失败重试、成功和返回库页路径仍缺截图。 | `docs/2026-05-04-artifact-to-runtime-gap-audit.md#_14-import-task-overlay` | 补有 pending task 的 importing/conflict/success/failure 截图；不升级运行态完成。 |
| `stitch_game_emulator_design_plan/_15` | `entry/src/main/ets/pages/OnboardingPage.ets` | 视觉完成 | 2026-05-04 对照：`02_boot_welcome_quick_start.png` 与设计同为欢迎 step 1；header、中心 icon、主 CTA、三列 status 可见，glass/scan-line 强度偏保守。 | 两步切换、授权入口和进入导入/库页路径继续绑定真实用户操作与 router。 | `LOCAL_ONLY` / `NOT_CONFIGURED` / `需真机验证`：已有 step 1 截图；step 2 授权态、按钮点击、router 跳转和动画仍需设备验证。 | `docs/2026-05-04-artifact-to-runtime-gap-audit.md#_15-onboarding` | 补 step 2 授权态、进入导入/库页路由截图；不升级运行态完成。 |
